...
describe Whatsapp::WebhookSetupService do
  ...
  before do
    ...
    # WhatsApp multi-number feature: all channels share one global verify token and one
    # generic callback URL (registered once in Meta), instead of a per-channel token/URL.
    allow_any_instance_of(described_class).to receive(:global_verify_token).and_return('test_verify_token')
  ...
  describe '#perform' do
      ...
      it 'registers the phone number and sets up webhook' do
        with_modified_env FRONTEND_URL: 'https://app.chatwoot.com' do
          ...
          expect(api_client).to receive(:subscribe_phone_number_webhook)
            .with(waba_id, '123456789', 'https://app.chatwoot.com/webhooks/whatsapp', 'test_verify_token',
    ...
    context 'when used during reauthorization flow' do
      ...
      it 'uses the shared global verify token during reauthorization' do
        with_modified_env FRONTEND_URL: 'https://app.chatwoot.com' do
          expect(api_client).to receive(:subscribe_phone_number_webhook)
            .with(waba_id, '123456789', anything, 'test_verify_token',
                  subscribed_fields: %w[messages smb_message_echoes])
