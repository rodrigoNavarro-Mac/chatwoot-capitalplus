# Refleja una llamada de Aircall como un Message real (content_type: 'voice_call') en la
...
#
# Solo procesa el evento `call.ended` (Aircall lo dispara ~30s después de colgar, ya con
# duración/grabación resueltas — ver developer.aircall.io/tutorials/webhooks-guide) y crea el
# `Call` directo en su estado terminal (completed/no_answer) en un solo paso — no hace falta
# suscribirse a `call.created`/`call.answered` ni modelar la transición ringing → in_progress,
# porque estas llamadas se reflejan en Chatwoot después de que ya terminaron, no en vivo.
#
