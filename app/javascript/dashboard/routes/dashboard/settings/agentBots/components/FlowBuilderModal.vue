...
const resolveTransitionType = w => {
  if (w.default) return 'default';
  if (w.contains) return 'contains';
  if (Object.prototype.hasOwnProperty.call(w, 'equals')) return 'equals';
  return 'starts_with';
};

const resolveTransitionValue = w => {
  if (w.default) return '';
  if (w.contains) return w.contains.join(', ');
  return w.equals ?? w.starts_with ?? '';
};
  ...
  try {
    ...
    steps.value = Object.entries(c.steps || {}).map(([name, s]) => ({
      ...
      transitions: (s.transitions || []).map(tr => {
        ...
        return {
          id: uid(),
          type: resolveTransitionType(w),
          value: resolveTransitionValue(w),
...
const generateConfig = () => {
  ...
  const config = {
    variables: vars,
    initial_step: initialStep.value,
    steps: stepsObj,
  };
...
const switchToVisual = () => {
  try {
    ...
    jsonError.value =
      'JSON inválido — revisa la sintaxis antes de cambiar a visual.';
  }
};

// ── Computed ──────────────────────────────────────────────────────────────────
const stepNames = computed(() =>
  steps.value.map(s => s.name.trim()).filter(Boolean)
);
const stepCount = computed(() => steps.value.filter(s => s.name.trim()).length);
const variableCount = computed(
  () => variables.value.filter(v => v.key.trim()).length
);

// ── Variables ─────────────────────────────────────────────────────────────────
const addVariable = () =>
  variables.value.push({ id: uid(), key: '', value: '' });
...
const updateStep = (i, updated) => {
  steps.value[i] = updated;
};
const setInitial = name => {
  initialStep.value = name;
};
...
<template>
  ...
  >
    <div class="flex flex-col gap-6">
      <!-- ── Tabs Visual / JSON ────────────────────────────────────────────── -->
      <div
        class="flex items-center gap-1 rounded-lg border border-n-weak bg-n-alpha-1 p-1 self-start"
      >
        <button
          type="button"
      ...
      <template v-if="activeTab === 'json'">
        <div class="flex flex-col gap-2">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-n-slate-9">
                Pega o edita el JSON del flujo directamente. Cambia a
                <strong>Visual</strong> para validar y convertir.
              </p>
            </div>
            <NextButton
              size="sm"
          ...
          <p
            v-if="jsonError"
            class="text-xs text-n-ruby-11 flex items-center gap-1"
          >
        ...
        <!-- Footer en modo JSON -->
        <div
          class="flex items-center justify-end gap-2 border-t border-n-weak pt-4"
        >
          <NextButton
            faded
            slate
      ...
      <template v-else>
        ...
        <section v-if="isEmpty" class="flex flex-col gap-3">
          <div>
            ...
            <p class="text-xs text-n-slate-9">
              Elige una plantilla lista para usar o empieza desde cero con los
              pasos de abajo.
            </p>
          </div>
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            >
              <span
                class="mt-0.5 h-5 w-5 shrink-0 text-n-blue-10"
                :class="tpl.icon"
              />
              <div class="min-w-0">
                ...
                <p class="mt-0.5 text-xs text-n-slate-9 leading-relaxed">
                  {{ tpl.description }}
                </p>
              </div>
            </button>
        ...
        <div
          v-else
          class="flex items-center justify-between rounded-lg border border-n-weak bg-n-alpha-1 px-3 py-2"
        >
          <p class="text-xs text-n-slate-9">
        ...
        <section class="flex flex-col gap-3">
          ...
          <p v-else class="text-xs italic text-n-slate-8">
            Sin variables — agrégalas si quieres personalizar mensajes.
          </p>
        </section>
        ...
        <section class="flex flex-col gap-3">
          <div class="flex items-center justify-between">
            <div>
              <h3 class="text-sm font-semibold text-n-slate-12">
                ...
                <span
                  v-if="stepCount"
                  class="ml-1.5 rounded-full bg-n-alpha-3 px-2 py-0.5 text-xs font-normal text-n-slate-10"
                >
                  {{ stepCount }}
                </span>
              </h3>
              <p class="text-xs text-n-slate-9">
                Paso inicial:
                <span class="font-mono font-medium text-n-blue-11">{{
                  initialStep || 'sin definir'
                }}</span>
              </p>
            </div>
            <NextButton
          ...
          >
            <template #item="{ element: step, index: i }">
              <FlowStep
                ...
                :is-initial="
                  initialStep === step.name.trim() && step.name.trim() !== ''
                "
                :index="i"
          ...
          <div
            v-else
            class="flex flex-col items-center gap-2 rounded-xl border border-dashed border-n-weak py-10 text-center"
          >
            <span class="i-lucide-git-branch h-8 w-8 text-n-slate-8" />
            <p class="text-sm text-n-slate-9">
              Sin pasos todavía.<br />Haz clic en
              <strong>Agregar paso</strong> para comenzar.
            </p>
          </div>
        </section>
        ...
        <section class="flex flex-col gap-3">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-n-slate-9">
                Si está activo, el bot consulta Zoho al inicio. Leads conocidos
                van al flujo principal; los desconocidos van al flujo de
                calificación.
              </p>
            </div>
            <button
          ...
          <div v-if="zohoCheck.enabled" class="flex flex-col gap-3">
            <div
              class="flex items-start gap-2 rounded-lg border border-n-blue-6 bg-n-blue-2 px-3 py-2"
            >
              <span
                class="i-lucide-info mt-0.5 h-4 w-4 shrink-0 text-n-blue-10"
              />
              <p class="text-xs text-n-blue-11">
                Crea pasos de calificación (ej:
                <code class="font-mono">calificar_nombre</code>,
                <code class="font-mono">calificar_razon</code>,
                <code class="font-mono">calificar_timeline</code>) con el campo
                <strong>Capturar respuesta</strong> activado, y aquí indica cuál
                es el primero.
              </p>
            </div>
            <div class="flex items-center gap-3">
              <label class="shrink-0 text-xs font-semibold text-n-slate-11"
                >Si no está en Zoho, iniciar en:</label
              >
              <select
          ...
          <div
            v-else
            class="rounded-lg border border-dashed border-n-weak px-4 py-3"
          >
            <p class="text-xs italic text-n-slate-8">
              Verificación Zoho desactivada — todos los leads entran al flujo
              principal.
            </p>
          </div>
        </section>
        ...
        <section class="flex flex-col gap-3">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-n-slate-9">
                Se envía automáticamente antes de que cierre la ventana de 24h
                de Meta si el lead no respondió.
              </p>
            </div>
            <!-- Toggle -->
            <button
            ...
            >
              <span
                class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out"
                :class="
                  reactivation.enabled ? 'translate-x-4' : 'translate-x-0'
                "
              />
            </button>
          </div>
          ...
          <div v-if="reactivation.enabled" class="flex flex-col gap-2">
            <!-- Timing info -->
            <div
              class="flex items-start gap-2 rounded-lg border border-n-amber-6 bg-n-amber-3 px-3 py-2"
            >
              <span
                class="i-lucide-clock mt-0.5 h-4 w-4 shrink-0 text-n-amber-11"
              />
              <p class="text-xs text-n-amber-11">
                El sistema manda este mensaje si el lead lleva entre
                <strong>20 y 23 horas</strong> sin responder — dejando margen
                antes del cierre de la ventana de Meta. Solo se manda
            ...
            <p class="text-[11px] text-n-slate-9">
              ...
              <code class="rounded bg-n-alpha-3 px-1 font-mono"
                >&#123;&#123;variables&#125;&#125;</code
              >
              del flujo. El mensaje debe ser breve y con una pregunta clara para
              que el lead responda.
            </p>
          </div>
        ...
        <!-- ── Footer ─────────────────────────────────────────────────────────── -->
        <div
          class="flex items-center justify-between border-t border-n-weak pt-4"
        >
          <p class="text-xs text-n-slate-9">
            ...
            <span
              :class="
                reactivation.enabled ? 'text-n-blue-10 font-medium' : 'italic'
              "
            >
