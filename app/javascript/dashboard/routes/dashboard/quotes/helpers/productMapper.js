...
// Color viene de "Colometria", un Lookup a otro módulo (no un texto plano) — Zoho lo devuelve como
// {id, name}. Fecha_de_entrega sí existe en Products y se usa como default editable.
export function mapZohoProductToFields(product) {
  return {
    lote: product.Product_Name || '',
    desarrollo: product.Desarrollo || '',
    color: product.Colometria?.name || '',
